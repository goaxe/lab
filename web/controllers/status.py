...
def repo(page):
    ...
    cur = uid
    return render_template('status/repo_status.html', cur=cur, repo_url=repo_url, departs=departs, count=count, failed=failed, suc=suc, now=now)
...
def upload_speed():
    ...
    departs = Department.query
    return render_template('status/upload_speed.html', data=json.dumps(data), departs=departs, uid=uid,
                           start=start, end=end)
...
def disk():
    ...
    departs = Department.query
    # status = DiskStatus.query
    # client_info = status.order_by(DiskStatus.update_time.desc()).first()
    # su = client_info.used * 1000000
    # st = client_info.total * 100
    # print su, st
